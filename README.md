...
- **Known issue: broad Mathlib-scale roots can fail to load in phase 2.** The cross-file environment is built by a runtime `importModules`, which cannot run `initialize` blocks that exist only as native code in modules outside the server's startup import closure — common in Mathlib's pp/tactic layers — and fails with an interpreter error (`cannot evaluate [init] declaration ...`). Targeted roots (specific module subtrees via `leanSuggest.roots`) work; a root importing all of Mathlib may not. The error message says how to narrow.
